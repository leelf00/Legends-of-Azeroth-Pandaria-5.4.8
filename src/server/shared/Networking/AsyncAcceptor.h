...
public:
    ...
    bool Bind()
    {
        ...
        if (errorCode)
        {
            TC_LOG_INFO("network", "Failed to open acceptor {}", errorCode.message().c_str());
            return false;
        }
        ...
        if (errorCode)
        {
            TC_LOG_INFO("network", "Failed to set reuse_address option on acceptor {}", errorCode.message().c_str());
            return false;
        }
#endif
        ...
        if (errorCode)
        {
            TC_LOG_INFO("network", "Could not bind to {}:{} {}", _endpoint.address().to_string().c_str(), _endpoint.port(), errorCode.message().c_str());
            return false;
        }
        ...
        if (errorCode)
        {
            TC_LOG_INFO("network", "Failed to start listening on {}:{} {}", _endpoint.address().to_string().c_str(), _endpoint.port(), errorCode.message().c_str());
            return false;
        }
