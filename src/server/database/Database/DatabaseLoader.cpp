...
DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, std::string const& name)
{
    ...
    _open.push([this, name, updatesEnabledForThis, &pool]() -> bool
    {
        ...
        if (dbString.empty())
        {
            TC_LOG_ERROR(_logger, "Database {} not specified in configuration file!", name.c_str());
            return false;
        }
    ...
    if (updatesEnabledForThis)
    {
        _populate.push([this, name, &pool]() -> bool
        {
            ...
            {
                TC_LOG_ERROR(_logger, "Could not populate the {} database, see log for details.", name.c_str());
                return false;
            }
            return true;
        });
        ...
        _update.push([this, name, &pool]() -> bool
        {
            ...
            {
                TC_LOG_ERROR(_logger, "Could not update the {} database, see log for details.", name.c_str());
                return false;
            }
            return true;
        });
    ...
    _prepare.push([this, name, &pool]() -> bool
    {
        ...
        {
            TC_LOG_ERROR(_logger, "Could not prepare statements of the {} database, see log for details.", name.c_str());
            return false;
        }
        return true;
    });
