...
class boss_high_astromancer_solarian : public CreatureScript
{
    public:
        ...
        struct boss_high_astromancer_solarianAI : public ScriptedAI
        {
            ...
            {
                Creature* Summoned = me->SummonCreature(entry, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000ms);
                if (Summoned)
                {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (Phase == 1)
                {
                    ...
                    if (Phase1_Timer <= diff)
                    {
                        ...
                        for (int i=0; i <= 2; ++i)
                        {
                            if (Creature* Summoned = me->SummonCreature(NPC_ASTROMANCER_SOLARIAN_SPOTLIGHT, Portals[i][0], Portals[i][1], Portals[i][2], CENTER_O, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(Phase2_Timer+Phase3_Timer+AppearDelay_Timer+1700)))
                            {
