...
class boss_grandmaster_vorpil : public CreatureScript
{
    public:
        ...
        struct boss_grandmaster_vorpilAI : public BossAI
        {
            ...
            {
                for (uint8 i = 0; i < 5; ++i)
                    if (Creature* portal = me->SummonCreature(NPC_VOID_PORTAL, VoidPortalCoords[i][0], VoidPortalCoords[i][1], VoidPortalCoords[i][2], 0, TEMPSUMMON_CORPSE_DESPAWN, 3000000ms))
            ...
            {
                uint8 pos = urand(0, 4);
                me->SummonCreature(NPC_VOID_TRAVELER, VoidPortalCoords[pos][0], VoidPortalCoords[pos][1], VoidPortalCoords[pos][2], 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000ms);
                if (!_helpYell)
                {
                    Talk(SAY_HELP);
                    _helpYell = true;
