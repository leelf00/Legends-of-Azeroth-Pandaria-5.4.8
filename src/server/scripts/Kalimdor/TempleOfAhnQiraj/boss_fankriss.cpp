    ...
    public:
        ...
        struct boss_fankrissAI : public BossAI
        {
            ...
            void SummonSpawn(Unit* victim)
            {
                ...
                Rand = 0;
                Spawn = DoSpawnCreature(15630, RandX, RandY, 0, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000ms);
                if (Spawn)
                        ...
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
                        {
                            ...
                            switch (urand(0, 2))
                            {
                                case 0:
                                    ...
                                    Hatchling = me->SummonCreature(15962, target->GetPositionX()-3, target->GetPositionY()-3, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000ms);
                                    if (Hatchling)
                                        Hatchling->AI()->AttackStart(target);
                                    Hatchling = me->SummonCreature(15962, target->GetPositionX()-3, target->GetPositionY()+3, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000ms);
                                    if (Hatchling)
                                        Hatchling->AI()->AttackStart(target);
                                    Hatchling = me->SummonCreature(15962, target->GetPositionX()-5, target->GetPositionY()-5, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000ms);
                                    if (Hatchling)
                                        Hatchling->AI()->AttackStart(target);
                                    Hatchling = me->SummonCreature(15962, target->GetPositionX()-5, target->GetPositionY()+5, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000ms);
                                    if (Hatchling)
