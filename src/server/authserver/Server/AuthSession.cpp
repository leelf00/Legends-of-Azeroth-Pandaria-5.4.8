...
void AuthSession::Start()
{
    ...
    TC_LOG_TRACE("session", "Accepted connection from {}", ip_address.c_str());
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            SendPacket(pkt);
            TC_LOG_DEBUG("session", "[AuthSession::CheckIpCallback] Banned ip '{}:{}' tries to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort());
            return;
        }
    }
...
bool AuthSession::HandleLogonChallenge()
{
    ...
    TC_LOG_DEBUG("server.authserver", "[AuthChallenge] '{}'", login.c_str());
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    {
        TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to IP - '{}' is logging in from '{}'", _accountInfo.Login.c_str(), _accountInfo.LastIP.c_str(), ipAddress.c_str());
    ...
    else
    {
        ...
        TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is not locked to ip", _accountInfo.Login.c_str());
        if (_accountInfo.LockCountry.empty() || _accountInfo.LockCountry == "00")
            TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is not locked to country", _accountInfo.Login.c_str());
        else if (!_ipCountry.empty())
        {
            TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to country: '{}' Player country is '{}'", _accountInfo.Login.c_str(), _accountInfo.LockCountry.c_str(), _ipCountry.c_str());
    ...
    if (_accountInfo.IsBanned)
    {
        if (_accountInfo.IsPermanenetlyBanned)
        {
            pkt << uint8(WOW_FAIL_BANNED);
            SendPacket(pkt);
            TC_LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Banned account {} tried to login!", ipAddress.c_str(), port, _accountInfo.Login.c_str());
            return;
        }
        else
        {
            ...
            SendPacket(pkt);
            TC_LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Temporarily banned account {} tried to login!", ipAddress.c_str(), port, _accountInfo.Login.c_str());
            return;
        }
    }
    ...
    if (!fields[9].IsNull())
    {
        ...
        //         TC_LOG_ERROR("server.authserver", "[AuthChallenge] Account '{}' has invalid ciphertext for TOTP token key stored", _accountInfo.Login.c_str());
        //         SendPacket(pkt);
        //         return;
        //     }
        // }
...
bool AuthSession::HandleLogonProof()
{
    ...
    if (std::optional<SessionKey> K = _srp6->VerifyChallengeResponse(logonProof->A, logonProof->clientM))
    {
        ...
        TC_LOG_DEBUG("server.authserver", "'{}:{}' User '{}' successfully authenticated", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo.Login.c_str());
...
bool AuthSession::HandleReconnectChallenge()
{
    ...
    std::string login((char const*)challenge->I, challenge->I_len);
    TC_LOG_DEBUG("server.authserver", "[ReconnectChallenge] '{}'", login.c_str());
...
bool AuthSession::HandleReconnectProof()
{
    ...
    {
        TC_LOG_ERROR("server.authserver.hack", "'{}:{}' [ERROR] user {} tried to login, but session is invalid.", GetRemoteIpAddress().to_string().c_str(),
...
void Patcher::LoadPatchMD5(char *szFileName)
{
    ...
    TC_LOG_DEBUG("network", "Loading patch info from {}\n", path.c_str());

    if (!pPatch)
    {
        TC_LOG_ERROR("server.authserver", "Error loading patch {}\n", path.c_str());
        return;
    }
