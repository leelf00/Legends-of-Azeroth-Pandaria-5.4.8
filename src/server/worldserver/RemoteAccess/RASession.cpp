...
void RASession::Start()
{
    ...
    TC_LOG_INFO("commands.ra", "Accepting RA connection from user {} (IP: {})", username.c_str(), GetRemoteIpAddress().c_str());
    ...
    TC_LOG_INFO("commands.ra", "User {} (IP: {}) authenticated correctly to RA", username.c_str(), GetRemoteIpAddress().c_str());
...
bool RASession::CheckAccessLevel(const std::string& user)
{
    ...
    if (!result)
    {
        TC_LOG_INFO("commands.ra", "User {} does not exist in database", user.c_str());
        return false;
    }
    ...
    {
        TC_LOG_INFO("commands.ra", "User {} has no privilege to login", user.c_str());
        return false;
    }
    else if (fields[2].GetInt32() != -1)
    {
        TC_LOG_INFO("commands.ra", "User {} has to be assigned on all realms (with RealmID = '-1')", user.c_str());
        return false;
    }
...
bool RASession::CheckPassword(const std::string& user, const std::string& pass)
{
    ...
    // TC_LOG_INFO("commands.ra", "Wrong password for user: {}", user.c_str());
...
bool RASession::ProcessCommand(std::string& command)
{
    ...
    TC_LOG_INFO("commands.ra", "Received command: {}", command.c_str());
