...
void CinematicMgr::NextCinematicCamera()
{
    ...
    if (std::vector<FlyByCamera> const* flyByCameras = GetFlyByCameras(cinematicCameraId))
    {
        ...
        if (!m_cinematicCamera->empty())
        {
            ...
            m_CinematicObject = player->SummonCreature(VISUAL_WAYPOINT, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 0.0f, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(5 * MINUTE * IN_MILLISECONDS));
