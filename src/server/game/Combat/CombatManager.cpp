...
{
    ASSERT(_pveRefs.empty(), "CombatManager::~CombatManager - %s: we still have %zu PvE combat references, one of them is %s", _owner->GetGUID().ToString().c_str(), _pveRefs.size(), _pveRefs.begin()->first.ToString().c_str());
    ASSERT(_pvpRefs.empty(), "CombatManager::~CombatManager - %s: we still have %zu PvP combat references, one of them is %s", _owner->GetGUID().ToString().c_str(), _pvpRefs.size(), _pvpRefs.begin()->first.ToString().c_str());
}
...
{
    for (auto const& pair : _pveRefs)
        if (!pair.second->IsSuppressedFor(_owner) && pair.second->GetOther(_owner)->GetTypeId() == TYPEID_PLAYER)
            return true;
    return false;
}
...
void CombatManager::InheritCombatStatesFrom(Unit const* who)
{
    ...
    for (auto const& pair : mgr._pveRefs)
    {
...
bool CombatManager::UpdateOwnerCombatState() const
{
    ...
    if (combatState)
        _owner->SetInCombatState(false); // PvE combat, non-PvP
    else
