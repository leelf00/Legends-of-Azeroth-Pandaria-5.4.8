...
uint32 ThreatManager::GetThreatListPlayerCount(bool includeOffline) const
{
    if (includeOffline)
...
Trinity::IteratorPair<ThreatManager::ThreatListIterator> ThreatManager::GetUnsortedThreatList() const
{
    ...
    return { ThreatListIterator{ std::move(generator) }, ThreatListIterator{ []() -> ThreatReference const* { return nullptr; } } };
}

Trinity::IteratorPair<ThreatManager::ThreatListIterator> ThreatManager::GetSortedThreatList() const
{
...
void ThreatManager::AddThreat(Unit* target, float amount, SpellInfo const* spell, bool ignoreModifiers, bool ignoreRedirects)
{
    ...
    if (spell)
    {
        if (spell->HasAttribute(SPELL_ATTR1_NO_THREAT))
            return;
        if (!_owner->IsInCombat() && spell->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO))
            return;
    }
    ...
    if (Unit* vehicle = target->GetVehicleBase())
    {
        ...
        if (target->HasUnitTypeMask(UNIT_MASK_ACCESSORY))
            return;
        amount = 0.0f;
    }
...
void ThreatManager::ProcessAIUpdates()
{
    ...
    std::vector<ObjectGuid> v(std::move(_needsAIUpdate));
    if (!ai)
        return;
...
{
    _owner->SendClearThreatListOpcode();
}

void ThreatManager::SendRemoveToClients(Unit const* victim) const
{
    _owner->SendRemoveFromThreatListOpcode(const_cast<Unit*>(victim));
}

void ThreatManager::SendThreatListToClients(bool newHighest) const
{
    std::vector<std::pair<Unit*, float>> threats;
    threats.reserve(_sortedThreatList->size());
    for (ThreatReference const* ref : *_sortedThreatList)
    {
        if (!ref->IsAvailable())
            continue;
        threats.emplace_back(ref->GetVictim(), ref->GetThreat());
    }

    if (newHighest)
        _owner->SendChangeCurrentVictimOpcode(_currentVictimRef->GetVictim(), threats);
    else
        _owner->SendThreatListUpdate(threats);
}
