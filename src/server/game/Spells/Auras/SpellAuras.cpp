...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false), _effMask(0)
{
    ...
    if (GetBase()->CanBeSentToClient(target))
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            TC_LOG_DEBUG("spells", "Aura: {} Effect: {} could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
    }
                    ...
                    case 12043: // Presence of Mind
                        ...
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
                        {
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                default:
                                    TC_LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency ({}) found", aurEff->GetId());
                            }
                            if (spellId)
            ...
            case SPELLFAMILY_GENERIC:
                switch (GetId())
                {
                    ...
                    case 75731: // Instant Statue
                        TC_LOG_INFO("spells", "[Q14465] ExitVehicle from SpellAuras 75731 for guid={}", target->GetGUID());
                        target->ExitVehicle();
