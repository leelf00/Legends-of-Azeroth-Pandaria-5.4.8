...
void QuestPoolMgr::LoadFromDB()
{
    ...
    // load template data from world DB
    {
        ...
        do
        {
            ...
            if (fields[2].IsNull())
            {
                TC_LOG_ERROR("sql.sql", "Table `pool_quest` contains reference to non-existing pool {}. Skipped.", fields[1].GetUInt32());
                continue;
            }
...
//            members[poolIndex].push_back(questId);
    ...
    for (auto const& pair : lookup)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `pool_template` contains quest pool {} requesting {} spawns, but only has {} members. Requested spawns reduced.", pool.poolId, pool.numActive, pool.members.size());
        ...
        if (!doRegenerate)
        {
            ...
            for (size_t i = pool.members.size(); (i--);)
            {
                ...
                if (member.empty())
                {
                    TC_LOG_ERROR("sql.sql", "Table `pool_quest` contains no entries at index {} for quest pool {}. Index removed.", i, pool.poolId);
