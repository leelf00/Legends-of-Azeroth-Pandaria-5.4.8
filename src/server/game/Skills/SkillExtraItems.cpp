...
void LoadSkillExtraItemTable()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Skill specialization {} has non-existent spell id in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Skill specialization {} have not existed required specialization spell id {} in `skill_extra_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Skill specialization {} has too low additional create chance in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!additionalMaxNum)
        {
            TC_LOG_ERROR("sql.sql", "Skill specialization {} has 0 max number of extra items in `skill_extra_item_template`!", spellId);
            continue;
        }
