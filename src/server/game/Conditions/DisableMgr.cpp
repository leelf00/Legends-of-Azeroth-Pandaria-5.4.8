        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_VMAP:
            {
                ...
                switch (mapEntry->map_type)
                {
                    ...
                    case MAP_RAID:
                        if (flags & VMAP_DISABLE_HEIGHT)
                            TC_LOG_INFO("misc", "Height disabled for raid map {}.", entry);
                        if (flags & VMAP_DISABLE_LOS)
                            TC_LOG_INFO("misc", "LoS disabled for raid map {}.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        if (flags & VMAP_DISABLE_HEIGHT)
            ...
            case DISABLE_TYPE_LFG_DUNGEON:
            {
                ...
                if (!lfgDungeon)
                {
                    TC_LOG_ERROR("sql.sql", "Lfg dungeon entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
            }
            default:
...
bool IsPathfindingEnabled(uint32 mapId)
{
    return sWorld->getBoolConfig(CONFIG_ENABLE_MMAPS)
        && !IsDisabledFor(DISABLE_TYPE_MMAP_MAP, mapId, nullptr, MMAP_DISABLE_PATHFINDING);
}
