...
bool AuctionBotBuyer::RollBuyChance(BuyerItemInfo const* ahInfo, Item const* item, AuctionEntry const* auction, uint32 /*bidPrice*/)
{
    ...
    if (ahInfo)
    {
        ...
        TC_LOG_DEBUG("ahbot", "AHBot: buyout average: {} items with buyout: {}", avgBuyPrice, ahInfo->BuyItemCount);
    ...
    TC_LOG_DEBUG("ahbot", "AHBot: {} BUY! chance = {}, price = {}, buyprice = {}.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBuyPrice));
    return win;
}
...
bool AuctionBotBuyer::RollBidChance(BuyerItemInfo const* ahInfo, Item const* item, AuctionEntry const* auction, uint32 bidPrice)
{
    ...
    if (ahInfo)
    {
        ...
        TC_LOG_DEBUG("ahbot", "AHBot: Bid average: {} biddable item count: {}", avgBidPrice, ahInfo->BidItemCount);
    ...
    float rand = frand(0.f, 100.f);
    bool win = rand <= chance;
    TC_LOG_DEBUG("ahbot", "AHBot: {} BID! chance = {}, price = {}, bidprice = {}.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBidPrice));
    return win;
}
...
{
    TC_LOG_DEBUG("ahbot", "AHBot: Entry {} bought at {}g", auction->Id, float(auction->buyout) / GOLD);
...
{
    TC_LOG_DEBUG("ahbot", "AHBot: Bid placed to entry {}, {}g", auction->Id, float(bidPrice) / GOLD);
