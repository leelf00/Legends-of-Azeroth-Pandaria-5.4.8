...
enum MovementGeneratorType : uint8
{
    ...
    FACE_MOTION_TYPE                = 19,
    NULL_MOTION_TYPE                = 20,    // no movement generator (legacy sentinel)
...
enum MovementSlot : uint8
{
    ...
    MOTION_SLOT_ACTIVE,
    MAX_MOTION_SLOT,
    MOTION_SLOT_IDLE = MOTION_SLOT_DEFAULT,
    MOTION_SLOT_CONTROLLED = 3,
    MOTION_SLOT_CRITICAL = 4
};
...
inline bool IsInvalidMovementSlot(uint8 const slot)
{
    return slot != MOTION_SLOT_DEFAULT && slot != MOTION_SLOT_ACTIVE &&
        slot != MOTION_SLOT_CONTROLLED && slot != MOTION_SLOT_CRITICAL;
}
